...
| D-017 | 2026-06-15 | Implement T-006 admin setup APIs through application use cases and a tenant-aware in-memory HTTP adapter until persistent repositories are introduced | Preserves clean architecture and lets admin CRUD contracts be validated before binding routes to PostgreSQL persistence | Admin endpoints require signed access tokens and admin role/tenant checks; later persistence work must replace the in-memory adapter without changing use-case boundaries |
| D-018 | 2026-06-15 | Implement order lifecycle through application use cases with an explicit `confirmed-items` API path | T-007 must support manual waiter/admin confirmed item insertion before the later voice-draft confirmation endpoint, while preserving that drafts cannot be submitted directly | Future voice and realtime tasks must call confirmation/order use cases instead of bypassing the explicit confirmed flow |
